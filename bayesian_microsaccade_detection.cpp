#include "bayesian_microsaccade_detection.h"
#include <iostream>
#include <fstream>
#include <sstream>
#include <ctime>
...
inline int integraltable::get_d_ind(const double d){ //implicitly casting
   return (d-0.1)/d_step_size-1; //UPDATE AS WE UPDATE the min d in the creation of integral table
}
...
integraltable::integraltable(char* filename){
  ...
  if(input_table.is_open()){
    getline(input_table, buff);//,  BUFFSIZE );
    ss << buff;
    ss >> Na;
    ss >> Nd;
...
void data::kalman_filter(const params& theta){
  ...
  for(int dim=0;dim<2;dim++){
    zf[dim][0]=x[dim][0];
    for(int i=0;i<T;i++){
      zf[dim][i]=zf[dim][i-1]+K*(x[dim][i]-zf[dim][i-1]);
      }
  }
  double Ks=P/(P+sigzsq);
  for(int dim=0;dim<2;dim++){
    z[dim][T-1]=zf[dim][T-1];
    for(int i=T-2;i!=-1;i--){
      z[dim][i]=zf[dim][i]+Ks*(z[dim][i+1]-zf[dim][i]);
    }
  }
  sum_dz_squared=0.0;
  for(int i=1;i<T;i++)
...
vector<bool> changepoints::get_vec(){
  ...
  for (int j=1;j<n;j++){ // n being the length of t01
...
data::data(char* filename){
  ...
  if(input.is_open())
  	for (int i=0;i<T;i++)
      input>>x[0][i]>>x[1][i]; //INSANE IT CAN READ LIKE THIS FROM A TXT FILE
  z[0]=new double[T];
  z[1]=new double[T];
    zf[0]=new double[T];
    zf[1]=new double[T];
...
bool MCMC_chain::make_step(const params& theta){
  ...
  if (n>=2){
    ...
    if (C.t10[k]-C.t01[k]==1){
      if (k!=n-1){
        ...
        comp=-log(n-1)+log(n-2)+log(C.t01[k+1]-C.t10[k-1]-2); //maybe problem here---102914
  ...
  case 2: //select a C.t10 and make it C.t10+1
    if (n>=2){ //if we had this we would sometimes get n=2 and thus log(2-2)=-inf
  ...
  case 3: //  select a C.t10 and make it C.t10-1
    if (n>=2){
      ...
      if (C.t10[k]-C.t01[k]==1){ //if it eliminates a changepoint
        if (k!=0){ //this was a bug-had 1, remember in C 1 is 0
          deltaloglik=loglik_down(C.t10[k-1],C.t01[k+1])-loglik_down(C.t10[k-1],C.t01[k])-loglik_up(C.t01[k], C.t10[k])-loglik_down(C.t10[k],C.t01[k+1]);
...
void changepoints::execute_step(int step_type,int k, int pos){
  ...
  break;
  case 4: //add a new pair by adding a 0 in a seq of 1s
    t01.insert(t01.begin()+k+1,pos+1); //bug here: was pos
    t10.insert(t10.begin()+k,pos);// was pos+1
    n++;
    N--;
    break;
  case 5: //add a new pair by adding a 1 in a seq of 0s
...
void parallel_tempered_chain::update_params(){
  cout<<"update_params"<<endl;
  for(int i=0;i<Nchains;i++)
    if(chain[i]){
    cout<<"if_chain_i"<<endl;
...
void parallel_tempered_chain::estimate_dsigma_up(){
  ...
  //for(alld and sigma on the grid){
  for(int i=0;i<nsamples;i++){
    for(int di=0;di<100;di++){  //100 points on grid both for d and sigma
  	  d=0.1+di*(5.0-0.1)/100.0;
  	  for(int sgi=0;sgi<100;sgi++){  //sigmag should be on log space--between 10^(-4) and 10^(-1)
...
void parallel_tempered_chain::estimate_sigmazx(){
  ...
  for(int i=0;i<nsamples;i++){
    ...
    if (abs(intercept-0.0)<0.00001) //added in sept 3
       intercept=0.00001;
...
parallel_tempered_chain::parallel_tempered_chain(double beta[], char* filename_dat, char* filename_Creal, char* filename_table):
 dat(filename_dat), C_real(filename_Creal,dat.T), table(filename_table){
  for(int i=0;i<Nchains;i++)
    chain[i]=NULL;
...
/*  double beta[82]={0.0,0.0005,0.001,0.0015,0.002,0.003, 0.004,0.005,0.006,0.007,0.0085,0.01,0.0115,
                   ...
                   2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 2.9, 3.0, 3.2, 3.4, 3.6, 3.8, 4.0, 4.25, 4.5};
*/ 
double beta[1]={1.0};

  if(argc<6){
    cout<<"I need at least 5 arguments:"<<endl
    <<"x_input_file\tC_input_file\tintegral_table\tparams_out\tchangepoints_out\t"<<endl;
    return 1;
  }
  parallel_tempered_chain mychains(beta, argv[1], argv[2], argv[3]);
  mychains.estimate_allthethings(argv[4],argv[5]);
  //if(argc==7)
  //  mychains.write_posteriors(100,argv[6],argv[7]);
  return 0;
}
