...
struct integraltable{
  double* table;//1-D array to index a 2-D space.
  double* a_upper; //the limit of a above which we can use the inf approx
  double* a_lower; // the limit of a below which we can use the 0 approx
  int Nd;
  int Na;
  double d_step_size;
  integraltable(char*);
...
class parallel_tempered_chain{
  public:
    ...
    int nsamples;
    parallel_tempered_chain(double*,char*,char*,char*);
